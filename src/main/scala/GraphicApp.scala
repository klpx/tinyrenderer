...
class Viewport extends JPanel {
  private val _width = 320
  private val _height = 240
    ...
    for(face <- faces) {
      for (edge <- face.edges) {
        val x0: Int = ((edge._1.x+1) * _width / 2).toInt
        val y0: Int = ((edge._1.y+1) * _height / 2).toInt
        val x1: Int = ((edge._2.x+1) * _width / 2).toInt
        val y1: Int = ((edge._2.y+1) * _height / 2).toInt
